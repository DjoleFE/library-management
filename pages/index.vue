<template>
  <div v-if="user" class="pb-4">
    <v-container class="mt-16">
      ...
      <v-row>
        <v-col cols="12" md="10" offset-md="1">
          <v-card title="Books" flat>
            <template v-slot:text>
              <v-text-field v-model="search" label="Search" prepend-inner-icon="mdi-magnify" variant="outlined"
                hide-details single-line></v-text-field>
            </template>
            <v-data-table :headers="headers" :items="filteredBooks" :search="search" item-key="id" items-per-page="5"
              class="elevation-1">
...
const filteredBooks = computed(() => {
  ...
  const userBooks = books.value.filter(
    (book) => book.user === currentUser.value.email
  );
