<template>
  <div class="login-page__wrapper fill-height">
      ...
      <v-card class="login-page__card" rounded="lg">
        ...
        <v-card-text class="px-8 pb-8">
          <v-form @submit.prevent="handleLogin" ref="form">
            <v-text-field class="mb-2" v-model="email" label="Email" type="email" :rules="[rules.required, rules.email]"
              required></v-text-field>
            <v-text-field class="mb-2" v-model="password" label="Password" type="password" :rules="[rules.required]"
              required></v-text-field>
    ...
    <v-alert v-if="showError" type="error" class="lm-alert position-fixed" dismissible color="#C51162"
      icon="mdi-alert-circle" theme="dark">{{ errorMessage }}</v-alert>
  </div>
</template>
